...
import cn.newtouch.application.security.service.AuthorityService;
//import cn.newtouch.application.security.service.RoleService;
import cn.newtouch.application.security.service.UserService;
import cn.newtouch.framework.utils.Digests;
import cn.newtouch.framework.utils.EncodeUtils;
	...
	protected AuthorizationInfo doGetAuthorizationInfo(PrincipalCollection principals) {
		...
		try {
			if (!subject.isAuthorized()) {
				...
				if (1 == subject.getUser().getAdmin()) {
					authorities = authorityService.findAllName();
				} else {
					Long userId = subject.getUser().getId();
					authorities = authorityService.findNameByUserId(userId);
				}
				subject.setAuthorities(authorities);
				subject.setRoles(new ArrayList<String>());
	...
	protected AuthenticationInfo doGetAuthenticationInfo(AuthenticationToken token) throws AuthenticationException {
		...
		try {
			...
			if (!user.getPosts().isEmpty()) {
				user.setCurrPost(user.getPosts().get(0));
			}
		} catch (Exception ex) {
			log.warn("获取用户失败\n" + ex.getMessage());
		}
