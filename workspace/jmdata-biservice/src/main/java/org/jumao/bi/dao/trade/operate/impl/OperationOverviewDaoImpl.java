...
public class OperationOverviewDaoImpl implements IOperationOverviewDao {
    ...
    public List<Map<String, Object>> getTopTotal(String platform, String nowDate) {
        ...
        String SQL = "";
        if (platform_ != null){
            SQL = "select count(distinct id) as nums from jmbi_uc_jumore_user   " +  //  where  create_time <= '"+nowDate+"'
                    " union all " +
                    ...
                    " union all " +
                    " select count(distinct id) as nums  from jmbi_ep_trade where industry_id= "+platform_+" AND  status in (2,3,4)  " +  //  is_delete = 0 and
                    " union all " +
                    " select sum(pay_amt) as nums  from jmbi_ep_trade  where industry_id= "+platform_+" AND   status in (2,3,4) ";
        }else {
            SQL = "select count(distinct id) as nums from jmbi_uc_jumore_user " +  //  platform = '" + platform + "' and
                " union all " +
                ...
                " union all " +
                " select count(distinct id) as nums  from jmbi_ep_trade where  status in (2,3,4)  " +
                " union all " +
                " select sum(pay_amt) as nums  from jmbi_ep_trade  where  status in (2,3,4)  ";
        }
