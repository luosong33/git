...
public class RegisterBasic extends RegisterHelper {
     ...
     * 处理折线图、柱状图
     */
    protected ChartResp getLineChartFromNewRegs(List<TimeTotalVo> list, String legend,
                                                boolean formatToNhAnd_, boolean accurateToHour) {
        ...
        for (int i = 0; i < size; i++) {
            try {
                ...
                xAxisData[i] = parseTime(ttv.getCreateTime(), formatToNhAnd_, accurateToHour);
...
//        mdcp.setSeries(series);
    ...
    protected ChartResp getTableFromNewRegs(List<DataSrcIncrTable> list, long totalUser,
        ...
        cp.setPercent(dealRatePercent(totalUser, sumCounts));
        return cp;
    }
