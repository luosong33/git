...
public class MediaUtils {
	...
	private static LogHelper logHelper = LogHelper.getLogger(MediaUtils.class);
	...
	public static String saveImage(MultipartFile multipartFile, int width,
		...
		logHelper.getBuilder().info("压缩图片尺寸："+width+" x "+height);
		SimpleDateFormat formater = new SimpleDateFormat("yyyy/MM/dd");
		String path = "upload/" + formater.format(new Date()) + "/"
				+ UUID.randomUUID().toString().replaceAll("-", "") + ".jpg";
		String filePath = PropertyUtils.getRoot() + "/" + path;
		File file = new File(PropertyUtils.getRoot() + "/" + path);
	...
	public static String save(MultipartFile multipartFile) throws IllegalStateException, IOException {
		SimpleDateFormat formater = new SimpleDateFormat("yyyy/MM/dd");
		String path = "upload/" + formater.format(new Date()) + "/file/"
				+ UUID.randomUUID().toString().replaceAll("-", "")
