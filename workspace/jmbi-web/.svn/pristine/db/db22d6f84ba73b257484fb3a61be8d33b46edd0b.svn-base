...
(function($){
  ...
  $.fn.tagsManager = function (options,tagToManipulate) {
    ...
    var deleteTag = function(tagId){
    	var tagValue = $("#" + objName + "_" + tagId).find("span").html();
    	$("#" + objName + "_" + tagId).remove();
    	if(tagManagerOptions.afterDelete != null && typeof(tagManagerOptions.afterDelete) == 'function'){
    		tagManagerOptions.afterDelete(tagValue);
  	      //console.log("deleteTag");
        }
    }
    
    //在删除标签后，在隐藏的input里面重新排列value
    ...
    var pushTag = function (tag) {
      ...
      var  tlisLowerCase = jQuery.map(tlis, function(elem) {
      ...
      if (alreadyInList) {
        ...
        //控制刷新时不执行
        if (tagManagerOptions.AjaxPush != null && !isRefresh) {
        	console.log("sengTag");
