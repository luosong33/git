...
function fileUpload(param) {
	...
	var language = "cn";
	var domain = "";
	var uploadId,imgId,filePathId,webFilePath,fileType,callback,onSelect,fileQueue="fileQueue",fileSize = 3,
		ifyWidth=50,ifyHeight=31,buttonImg=null,fileQueueAuto=true,queueSizeLimit=6,fileMulti=false,simUploadLimit=1;
	if(param){
	...
	if (typeof value != 'undefined') { // name and value given, set cookie
		...
		if (document.cookie && document.cookie != '') {
			...
			for (var i = 0; i < cookies.length; i++) {
				...
				if (name && name.length && cookie.substring(0, name.length + 1) == (name + '=')) {
